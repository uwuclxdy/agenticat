...
description: This skill should be used when updating the ratatui-pro skill and agent to a new ratatui release (refresh the bundled api-reference, modernization checklist, and limitations after a ratatui version bump, or when asked to check whether ratatui-pro is stale).
disable-model-invocation: true
---

# ratatui-pro-update

Refreshes this repo's `skills/ratatui-pro/` + `agents/ratatui-pro.md` after a ratatui
release. Run it whenever crates.io is ahead of the version pinned in ratatui-pro's SKILL.md.
...
1. **Detect.**
   ...
   version at the top of `skills/ratatui-pro/SKILL.md` and in `references/api-reference.md`'s header.
   Equal → report "current" and stop.
   Stale-check-only ask (e.g. "is ratatui-pro stale?", not a full update) → still run step 2 to
...
4. **Sweep for drift.** Grep old version strings + every changed public symbol across all of
   `skills/ratatui-pro/`, `.claude/skills/ratatui-pro-update/SKILL.md`, and `agents/ratatui-pro.md`;
   fix stale mentions.
...
6. **Optionally re-audit** the fleet: `ratatui-pro/references/audit-prompt.md` has the
