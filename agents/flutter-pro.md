...
2. Survey. Read the surrounding feature: state-management wiring, error strategy, folder layout, `analysis_options.yaml`, where tests live. Match what's already there instead of importing a new pattern. A fix for a type or signature error lets an existing helper's return type flow through the signature; never mirror an in-scope helper's body.
