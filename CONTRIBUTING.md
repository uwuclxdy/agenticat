...
- **Nothing personal or machine-specific**: absolute paths, IPs, hostnames, private tooling. `grep -rE '/home/|192\.168\.' agents/ skills/` must come back clean.
...
- Frontmatter: `name`, `description`, `tools` (comma-separated string), `model` (`haiku`/`sonnet`/`opus` alias). Frontmatter values stay lowercase; body headings use Title Case (`## Method`).
