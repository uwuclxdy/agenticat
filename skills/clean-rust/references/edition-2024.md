...
- **References to `static mut` are rejected.** Reach for atomics, `Mutex`, or `OnceLock`; raw-pointer access (`&raw const X`) is the escape hatch for genuine FFI statics.
...
- **`env::set_var` / `env::remove_var` are `unsafe`** — mutating the process environment races other threads reading it (a real intermittent-test-failure source; see `testing.md`). Call only in single-threaded context, in an `unsafe {}` block.
...
- **`unsafe_op_in_unsafe_fn` warns by default**: bodies of `unsafe fn` need explicit inner `unsafe {}` blocks.
